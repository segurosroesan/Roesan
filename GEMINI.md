...
# Repositorio Oficial & Deploy (Abril 2026 - Presente)
CRITICAL: Este proyecto DEBE usar de ahora en adelante el entorno y credenciales propias del cliente (Roesan):
1. **GitHub Oficial:** `https://github.com/segurosroesan/Roesan.git` (rama `master` como despliegue a producción).
2. **Despliegue:** Netlify App gestionada por `segurosroesan` y configurado con Zero-Config (sin `netlify.toml`).
3. **Nota sobre Commits:** Siempre documentar el por qué y para qué en el commit y asegurar que no rompimos el stack visual del cliente web (Animaciones Anime.js, layout oscuro, grid/UnicornBackground).
